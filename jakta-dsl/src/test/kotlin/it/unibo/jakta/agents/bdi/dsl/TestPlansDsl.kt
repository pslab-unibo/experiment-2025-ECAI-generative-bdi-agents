    ...
    DescribeSpec({
        ...
        describe("A test trigger plan") {
            it("should be created with an invocation trigger") {
                val plan =
                    testingPlans {
                        +test("send_ping".fromSelf) then { }
                    }.first()
                val belief = Belief.wrap(Atom.of("send_ping"), wrappingTag = SOURCE_SELF)
                plan.trigger.value.equals(belief.rule.head, false) shouldBe true
